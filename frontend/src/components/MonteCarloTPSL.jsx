...
export default function MonteCarloTPSL() {
  ...
  const sigStyle = { FAVORABLE: 'bg-emerald-900/30 text-emerald-400', UNFAVORABLE: 'bg-red-900/30 text-red-400', NEUTRAL: 'bg-gray-800 text-gray-400' };

  return (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      <h3 className="text-lg font-bold text-white mb-1">🎲 TP/SL Probability Engine</h3>
      <p className="text-xs text-gray-600 mb-4">5,000 Monte Carlo simulations — what are the odds?</p>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <input type="number" value={tp} onChange={e => setTp(e.target.value)}
            className="bg-gray-800 text-gray-300 text-xs rounded px-2 py-1.5 w-14" />
        ...
        <button onClick={run} disabled={loading}
          className="bg-indigo-600 text-white text-xs px-4 py-1.5 rounded hover:bg-indigo-700 disabled:opacity-50">
      ...
      {data && (
        <>
          <div className="flex items-center justify-between mb-3">
            <span className="font-mono text-sm text-gray-300">{data.symbol} @ ${data.current_price}</span>
          ...
          <div className="mb-4">
            <div className="flex h-8 rounded-lg overflow-hidden">
              <div className="bg-emerald-600 flex items-center justify-center" style={{ width: `${data.tp_probability}%` }}>
                <span className="text-xs text-white font-bold">{data.tp_probability}%</span>
              </div>
              {data.neither_pct > 5 && (
                <div className="bg-gray-700 flex items-center justify-center" style={{ width: `${data.neither_pct}%` }}>
                  <span className="text-xs text-gray-400">{data.neither_pct}%</span>
                </div>
              )}
              <div className="bg-red-600 flex items-center justify-center" style={{ width: `${data.sl_probability}%` }}>
                <span className="text-xs text-white font-bold">{data.sl_probability}%</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            ...
            <div className="bg-gray-800/40 rounded-lg p-2 text-center">
              <p className="text-xs text-gray-500">Median to SL</p>
              <p className="font-mono text-red-400 font-bold">{data.median_days_to_sl ?? '—'}d</p>
            </div>
            <div className="bg-gray-800/40 rounded-lg p-2 text-center">
              <p className="text-xs text-gray-500">Expected P&L</p>
              <p className={`font-mono font-bold ${data.expected_pnl_pct >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {data.expected_pnl_pct > 0 ? '+' : ''}{data.expected_pnl_pct}%
              </p>
            </div>
            <div className="bg-gray-800/40 rounded-lg p-2 text-center">
              <p className="text-xs text-gray-500">Annual Vol</p>
